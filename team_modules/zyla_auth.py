...
from z_authentication_module.utils.db import users_col
from z_authentication_module.utils.auth_helpers import hash_password, verify_password, generate_otp, otp_expiry
from z_authentication_module.utils.firebase_admin_init import *
...
# ----------------------
@appAuth.post("/check-google-user")
def check_google_user(data: GoogleEmailCheck):
...
# ----------------------
@appAuth.post("/login")
def login(data: LoginModel, request: Request = None):
    ...
    if "@" in identifier:
        ...
        user = users_col.find_one({"phone": identifier})
        method = "phone"
    ...
    return {"message": "Login successful", "method": method}

# ----------------------
# SEND EMAIL OTP (returns expiry)
# ----------------------
@appAuth.post("/send-email-otp")
...
@appAuth.post("/phone-otp-attempt")
def phone_otp_attempt(data: PhoneOtpAttempt):
...
# ----------------------
@appAuth.post("/reset-password-phone")
def reset_password_phone(data: ResetPasswordPhone):
...
# ----------------------
# ----------------------
@appAuth.post("/reset-password-email")
def reset_password_email(data: ResetPasswordEmail):
